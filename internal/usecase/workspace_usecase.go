...
func (u *workspaceUsecase) GetWorkspaceSummary(workspaceID int, period string) (map[string]float64, error) {
	data, err := u.workspaceRepo.CalculateSummary(workspaceID)
	if err != nil {
		return nil, err
	}

	// Masukin parameter period ke sini, JANGAN di-hardcode "2026-05" lagi
	limit, target, err := u.workspaceRepo.GetBudgetByWorkspace(workspaceID, period)
	if err != nil {
		limit, target = 0, 0
	}
